...
class ConnectApp extends Component<Props, State> {
  ...
  handleHeadersSubmit = () => {
    ...
    if (isConnected) {
      const { pastApps } = JSON.parse(getLocalStorageItem(LOCAL_CONNECTIONS));
        ...
        {isShowingAppSwitcher && (
          <Form layout="inline" onSubmit={this.handleSubmit}>
            <Flex alignItems="center">
              <Item {...formItemProps} css={{ [mediaMin.medium]: { flex: 1 } }}>
                <Group compact css={{ display: 'flex !important' }}>
                  <Input
                    name="url"
                    value={url}
                    ...
                    css={{
                      color:
                        isUrlHidden && isConnected && 'transparent !important',
                    }}
                  />
                  <Button
                    css={{
                  ...
                  >
                    <i
                      className={`fa ${
                        isUrlHidden ? 'fa-eye-slash' : 'fa-eye'
                      }`}
                    />
                  </Button>
                  <Button
                    ...
                    >
                      ...
                      >
                        <Input
                          value={item.key}
                          onChange={e =>
                            this.handleHeaderItemChange(e, i, 'key')
                          }
                        />
                      </div>
                      <div
                      ...
                      >
                        <Input
                          value={item.value}
                          onChange={e =>
                            this.handleHeaderItemChange(e, i, 'value')
                          }
                        />
                      </div>
                      <div
        ...
        {!isLoading && !isConnected && (
          <Alert
            ...
            description={
              <React.Fragment>
                <div>
                  <h3 style={{ marginTop: '1rem' }}>Connection Tips</h3>
                  <ul>
                    <li>
                      You can connect to all indices by passing an{' '}
                      <code>*</code> in the app name input field.
                    </li>
                    <li>
                      You can also connect to a single index or multiple indices
                      by passing them as comma separated values: e.g.
                      index1,index2,index3.
                    </li>
                    <li>
                      Avoid using a trailing slash <code>/</code> after the
                      cluster address.
                    </li>
                    <li>
                      Your cluster needs to have CORS enabled for the origin
                      where Dejavu is running. See below for more on that.
                    </li>
                  </ul>
                  <h3>CORS Settings</h3>
                  <p>
                    To make sure you have enabled CORS settings for your
                    ElasticSearch instance, add the following lines in the ES
                    configuration file:
                  </p>
                  <pre>
                    {`http.port: 9200
...
http.cors.allow-credentials: true`}
                ...
                <div style={{ marginTop: '2rem' }}>
                  <p>
                    If you are running ElasticSearch via Docker, use the
                    following command:
                  </p>
                  <div
                    style={{
                  ...
                  >
                    <code>
                      docker run --name es -d -p 9200:9200 -e http.port=9200 -e
                  ...
                  <p style={{ marginTop: '14px' }}>
                    Or the following if you are using v7 ElasticSearch:
                  </p>
                  <div
                    style={{
                      background: '#fefefe',
                  ...
                  >
                    <code>
                      docker run -d --rm --name elasticsearch -p 9200:9200 -p
                      9300:9300 -e discovery.type=single-node -e
                      http.cors.enabled=true -e
                      http.cors.allow-origin=http://localhost:1358,http://127.0.0.1:1358
                      -e
                      http.cors.allow-headers=X-Requested-With,X-Auth-Token,Content-Type,Content-Length,Authorization
